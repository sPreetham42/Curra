...
func TestBacktrackingSolverFindsFeasibleSolution(t *testing.T) {
	...
	testutil.AssertAllRequiredSessionsScheduled(t, &p, &solution)
}
...
func TestBacktrackingSolverReturnsCapacityDiagnosticsWhenUnsatisfied(t *testing.T) {
	...
	if !testutil.HasViolation(diag.Violations, "RoomCapacity") {
...
func TestProblemValidationRejectsInvalidStructure(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !testutil.HasViolationMessageContaining(violations, tt.wantPhrase) {
...
func TestDisjointSubgroupsMayRunTogetherButWholeGroupConflicts(t *testing.T) {
	...
	if violations := constraints.CheckAll(&p, &solution, whole, constraints.DefaultHardConstraints()); !testutil.HasViolation(violations, "StudentGroupConflict") {
...
func TestHeuristicsDrivesSearch(t *testing.T) {
	...
	testutil.AssertAllRequiredSessionsScheduled(t, &p, &basicSol)
	...
	testutil.AssertAllRequiredSessionsScheduled(t, &p, &heurSol)
...
func TestValidateSolution_LegacyFallbackDetectsConstraintViolation(t *testing.T) {
	p := feasibleProblem()
	p.Prepare()

	// Construct a solution with a known faculty conflict:
	// Both assignments use faculty-a at overlapping time slots.
	...
	a1 := problem.Assignment{
		...
		RoomID:               "room-lecture",
		TimeSlotID:           "mon-1",
		SessionRequirementID: "req-theory",
		Instance:             0,
	}
	...
	// Use AddAssignment which checks SolutionIndex conflicts.
	// a1 should succeed (first assignment).
	if err := sol.AddAssignment(&p, a1); err != nil {
		t.Fatalf("AddAssignment a1 failed: %v", err)
	}
	// a2 should fail because faculty-a is already at mon-1.
	if err := sol.AddAssignment(&p, a2); err == nil {
		t.Fatal("expected AddAssignment a2 to fail (faculty conflict), but it succeeded")
	}

	// Manually add a2 to the Assignments slice (bypassing index) to simulate a corrupted state.
	sol.Assignments = append(sol.Assignments, a2)

	// Now call ValidateSolution with a solver that has NO compiled constraints.
	solver := backtracking.New()
	_, diag, err := solver.ValidateSolution(nil, p, sol)

	// The legacy fallback must detect the faculty conflict.
	if err == nil {
		t.Fatal("expected ValidateSolution to return error for faculty conflict, got nil")
	}
	if diag.Status != diagnostics.SolveStatusInfeasible {
		t.Fatalf("expected INFEASIBLE, got %s", diag.Status)
	}
	if len(diag.Violations) == 0 {
		t.Fatal("expected at least one violation, got none")
	}
	if !testutil.HasViolation(diag.Violations, "FacultyConflict") {
		t.Fatalf("expected FacultyConflict violation, got %+v", diag.Violations)
	}
}

func TestValidateSolution_LegacyFallbackPassesValidSolution(t *testing.T) {
	p := feasibleProblem()
	p.Prepare()

	// Solve with New() — search checks constraints during construction.
	solver := backtracking.New()
	...
	// Now call ValidateSolution on the result with no compiled constraints.
	_, diag2, err2 := solver.ValidateSolution(nil, p, sol)
	if err2 != nil {
		t.Fatalf("ValidateSolution failed: %v", err2)
	}
	if diag2.Status != diagnostics.SolveStatusSolved {
		t.Fatalf("expected SOLVED from ValidateSolution, got %s", diag2.Status)
	}
